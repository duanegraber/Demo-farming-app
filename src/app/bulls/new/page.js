...
export default function AddBullPage() {
  ...
  async function handleSubmit(event) {
    ...
    const bull = await createBull(form);
    router.push(`/bulls/${bull.id}`);
  }
  ...
  return (
    <main className="app-shell">
      <header className="page-header">
        ...
        <h1>Add Bull</h1>
        <p className="muted">Add a herd bull by tag number or name.</p>
      </header>
      <form className="form-card" onSubmit={handleSubmit}>
        <label>Bull tag number<input required value={form.tag} onChange={(event) => updateField("tag", event.target.value)} placeholder="Example: B12" /></label>
        ...
        <button type="submit" className="button full">Save bull</button>
      </form>
    </main>
  );
}
