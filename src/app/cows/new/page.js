...
import { createCow, loadPastures } from "../../components/farmStore";

export default function AddCowPage() {
  const router = useRouter();
  const [pastures, setPastures] = useState([]);
  const [form, setForm] = useState({ tag: "", status: "Active", color: "", location: "", lastCalved: "", calfTag: "", purchaseCost: "", dateBought: "", sellingAmount: "", dateSold: "", notes: "", user: "Alex" });
  ...
  return (
    <main className="app-shell">
      <header className="page-header">
        ...
        <h1>Add Cow</h1>
        <p className="muted">Add a new cow by tag number. Saves on this device for now.</p>
      </header>
      <form className="form-card" onSubmit={handleSubmit}>
        <label>Cow tag number<input required value={form.tag} onChange={(event) => updateField("tag", event.target.value)} placeholder="Example: 83" inputMode="numeric" /></label>
        ...
        <label>Current calf tag<input value={form.calfTag} onChange={(event) => updateField("calfTag", event.target.value)} placeholder="Optional" inputMode="numeric" /></label>
        ...
        <button type="submit" className="button full">Save cow</button>
      </form>
    </main>
  );
}
